...
- Supports Twitch Chat integration where it can read chat messages and pick a random recent one (configurable how recent) instead of just being idle.
- Support using a local LLM model instead of using OpenAI, which will completely remove that cost although is harder to get good results.

## Audio Podcast Description (Generated by google notebooklm)
https://github.com/user-attachments/assets/d830272c-56ac-4cee-8daf-3ee16351790c
...
```json
{
    ...
    "twitch_access_token": "YOUR KEY HERE"
}
```
...
- If you want Twitch Chat integration (the character can respond to twitch chat) then you must create an Application on dev.twitch.tv, get its ClientID, then generate an access token for it to use and put it in the token_config.json.
  - To generate an access token using the client id you will have to open a browser and go to this url (replace YOUR_CLIENT_ID with yours)
...
## Using a local model instead of ChatGPT/OpenAI
- In Character Config defined above add an attribute:
```
"local_model_name": "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B",
```
- Replace the model with whatever one you would like that is available on huggingface.
- The first time you prompt the character it will download the model you have selected and load it
- The 1.5B model example is around 3.55GB.
- A large model like the 7B one below is over 15GB.
```
"local_model_name": "deepseek-ai/DeepSeek-R1-Distill-Qwen-7B",
```
- You will have to radically change how you write your system prompt, and the results from my experience are quite bad in comparison to chatgpt.
...
- `enable_twitch_integration`: If true will enable twitch integration and will attempt to connect to the configured twitch channel's chat.
- `twitch_channel_name`: The name of your twitch channel that it should listen to. Can be exluded if enable_twitch_integration is false.
- `twitch_chat_history_length`: The number of most recent twitch chat messages it should consider when picking what to read (1 would always be the latest message). Can be exluded if enable_twitch_integration is false.
