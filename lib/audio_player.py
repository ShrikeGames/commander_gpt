...
class AudioManager:
    ...
    def combine_audio_files(self, input_files):
        ...
        output_file = os.path.join(
            os.path.abspath(os.curdir), f"___Msg{str(hash(' '.join(input_files)))}.wav"
        )
        combined = None
    ...
    def record_audio(self, end_recording_key="=", audio_device=None):
        ...
        if audio_device is None:
            ...
            if self.rate is None:
                raise ValueError(
                    f"No supported sample rate found for device '{audio_device}'"
                )
            audio_stream = audio.open(
                format=self.audio_format,
                channels=self.channels,
        ...
        wait_until_key(key_to_match=end_recording_key)
